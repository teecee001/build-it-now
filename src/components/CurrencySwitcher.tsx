  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      toast.success(`Converted to ${pendingCurrency}`, {
        description: `${formatAmount(convertedAmount, pendingCurrency)} now in your ${pendingCurrency} wallet`,
      });
    } catch (err: any) {
      ...
      <Dialog open={!!pendingCurrency} onOpenChange={(open) => { if (!open && !isConverting) setPendingCurrency(null); }}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Your funds will be converted at the current exchange rate.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-4 py-2">
              ...
              <div className="flex items-center justify-center gap-3">
                <div className="flex flex-col items-center gap-1 p-3 rounded-xl bg-secondary/60 flex-1">
                  ...
                  <span className="text-sm font-semibold font-mono">{formatAmount(fromBalance, activeCurrency)}</span>
                </div>
                ...
                <div className="flex flex-col items-center gap-1 p-3 rounded-xl bg-accent/10 border border-accent/20 flex-1">
                  ...
                  <span className="text-sm font-semibold font-mono text-accent">{formatAmount(convertedAmount, pendingCurrency)}</span>
                </div>
              </div>
              ...
              <div className="rounded-lg bg-secondary/40 p-3 space-y-1.5">
                ...
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-mono font-medium">{formatAmount(fromBalance, activeCurrency)}</span>
                </div>
                <div className="flex items-center justify-between text-xs">
                  <span className="text-muted-foreground">You Receive</span>
                  <span className="font-mono font-semibold text-accent">{formatAmount(convertedAmount, pendingCurrency)}</span>
                </div>
                <div className="flex items-center justify-between text-xs">

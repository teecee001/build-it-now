...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }}>
          <Card className="p-4 bg-gradient-to-r from-warning/10 to-warning/5 border-warning/20 flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-muted-foreground">Credited to your Ξ╳oSky wallet</p>
            </div>
      ...
      {/* Demo path — live surfaces only */}
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.12 }}>
        <Card className="p-4 md:p-5 border-primary/30 bg-gradient-to-br from-primary/10 via-card to-card">
          <p className="text-[11px] uppercase tracking-wider text-muted-foreground font-medium">Live demo path</p>
          <p className="text-sm text-muted-foreground mt-1 mb-3">
            Show the real feeds first — skip everything that is still paper or gated.
          </p>
          <div className="flex flex-col sm:flex-row gap-2">
            <Button className="flex-1 justify-between" onClick={() => navigate("/markets")}>
              <span className="flex items-center gap-2"><Bitcoin className="w-4 h-4" /> 1. Markets · Crypto</span>
              <ArrowUpRight className="w-4 h-4" />
            </Button>
            <Button className="flex-1 justify-between" variant="secondary" onClick={() => navigate("/forecasts")}>
              <span className="flex items-center gap-2"><BarChart3 className="w-4 h-4" /> 2. Forecasts · model odds</span>
              <ArrowUpRight className="w-4 h-4" />
            </Button>
          </div>
        </Card>

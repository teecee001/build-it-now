...
const POS_KEY = "exosky-paper-positions-v1";
...
/** Resolve paper positions whose calendar clock has passed using current model odds. */
function settleDue(list: Position[], markets: ForecastMarket[]): Position[] {
  const now = Date.now();
  let changed = false;
  const next = list.map((pos) => {
    if (pos.status !== "open") return pos;
    const resolveMs = Date.parse(pos.resolvesAt);
    // resolvesAt is like "Sep 30, 2026" — parse may fail; use market resolvesInDays as backup
    const m = markets.find((x) => x.id === pos.marketId);
    const due =
      (Number.isFinite(resolveMs) && resolveMs + 86400000 < now) ||
      (m ? m.resolvesInDays <= 0 : false);
    if (!due) return pos;
    // Paper settle: YES wins if model yesPrice >= 50 at expiry (honest model-implied close)
    const yes = m?.yesPrice ?? pos.avgPrice;
    const yesWins = yes >= 50;
    const won = pos.side === "YES" ? yesWins : !yesWins;
    changed = true;
    return {
      ...pos,
      status: (won ? "won" : "lost") as Position["status"],
      settledAt: new Date().toISOString(),
      settleNote: m
        ? `Settled on model close · YES ${yes}¢`
        : "Market expired · settled without live book",
    };
  });
    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(i, 8) * 0.03 }}>
      <Card className="p-4 h-full hover:border-primary/40 transition-colors flex flex-col">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Badge variant="outline" className="text-[10px] border-amber-500/30 text-amber-600 dark:text-amber-400">Model odds</Badge>
          {m.spotLabel && (
...
export default function Forecasts() {
  ...
  // Persist + settle when market data refreshes
  useEffect(() => {
    const settled = settleDue(positions, markets);
    if (settled !== positions) {
      setPositions(settled);
      savePositions(settled);
      return;
    }
  ...
  const confirmTrade = () => {
    ...
    setPositions((prev) => {
      const next: Position[] = [...prev, {
        marketId: trading.market.id,
        side: trading.side,
        shares: Math.round(shares * 100) / 100,
        avgPrice: priceCents,
        question: trading.market.question,
        openedAt: new Date().toISOString(),
        resolvesAt: trading.market.resolvesAt,
        status: "open",
      }];
      savePositions(next);
      return next;
  ...
  const portfolioValue = openPos.reduce((sum, p) => {
    const yes = priceOf(p.marketId);
    const nowCents = p.side === "YES" ? yes : 100 - yes;
    return sum + p.shares * (nowCents / 100);
  }, 0);
  const portfolioCost = openPos.reduce((sum, p) => sum + p.shares * (p.avgPrice / 100), 0);
  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {counts.all} markets from live spots · model-implied odds · paper trades persist on this device.
          </p>
        </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-4">
            ...
            <div className="flex-1 md:text-center">
              <p className="text-sm text-muted-foreground">
                ...
                <span className="text-amber-600 dark:text-amber-400"> · model</span>
            ...
            <Card className="p-4 flex flex-wrap items-center gap-x-8 gap-y-2">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="font-medium">
                  {positions.filter((p) => p.status === "open").length} open · {positions.filter((p) => p.status !== "open").length} settled
                </span>
              </div>
              <div className="text-sm">
      ...
      <div className="flex items-start gap-2 text-xs text-muted-foreground border-t border-border pt-4">
        ...
        <p>
          Odds are <strong className="text-foreground">model-implied</strong> from live spots
          (ECB FX, CoinGecko, Fear &amp; Greed) using realized volatility — not an exchange order book.
          Paper positions save on this device and settle when the resolve date passes. No real funds move.
        </p>
      </div>
